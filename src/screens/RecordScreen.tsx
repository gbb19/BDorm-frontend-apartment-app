...
import { IBillCreateResponse } from "../types/bill.types";
...
export function RecordScreen() {
  ...
  async function fetchLedgerItem() {
    setLoading(true);
    try {
      const data = await LedgerController.getLedgerItemsByMonthAndYear(
        selectedMonth + 1,
        selectedYear,
        user?.token!
      );
  ...
  async function handleCreateLedger() {
    try {
      await LedgerController.createLedger(
        selectedMonth + 1,
        selectedYear,
        user?.username!,
        user?.token!
      );
      await LedgerController.createLedgerItem(
        selectedMonth + 1,
        selectedYear,
        101,
        0,
  ...
  const handleUpdate = async (updatedItem: LedgerItem) => {
    try {
      await LedgerController.updateLedgerItem(
        updatedItem.ledgerMonth,
        updatedItem.ledgerYear,
        updatedItem.ledgerItemRoomNumber,
        updatedItem.waterUnit,
        updatedItem.electricityUnit,
        user?.token!
      );
      fetchLedgerItem();
    } catch (error) {
  ...
  const handleIssue = async (item: LedgerItem) => {
    try {
      await LedgerController.updateLedgerItemStatus(
        item.ledgerMonth,
        item.ledgerYear,
        item.ledgerItemRoomNumber,
        user?.token!
      );
      const billID = await BillController.createBill(
        5,
        item.contract.username,
        user?.username!,
        user?.token!
      );
      ...
      <ScrollView style={styles.content}>
        ...
        {loading ? (
          ...
          ledgerItems?.map((ledgerItem, index) => (
            <RecordCard
              key={ledgerItem.ledgerItemRoomNumber}
